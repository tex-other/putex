...
#if	(OS_ATARI || OS_BSD || OS_BS2000 || OS_MVSXA || OS_PCDOS || OS_VMCMS || OS_WIN32)
#else
...
#if    (CCD_2000 || OS_SYSV || OS_PCDOS || STDC || _AIX || ardent || OS_WIN32)
#include    <string.h>
#else
...
#endif /* (CCD_2000 || OS_SYSV || OS_PCDOS || STDC || _AIX || ardent || OS_WIN32) */

#if    OS_PCDOS || OS_WIN32
...
#if    OS_PCDOS || OS_WIN32                        /* open file in binary mode */
...
#ifdef DBCS
	unsigned char	*sksf[FIELD_MAX];	/* sort order of sort key */
	dbchar	*dbsf[FIELD_MAX];	/* sort order of sort key */
	unsigned char	*skaf[FIELD_MAX];	/* sort order of actual key */
	dbchar	*dbaf[FIELD_MAX];	/* sort order of actual key */
#endif
    int     group;			/* key group */
...
#ifdef DBCS
	unsigned char	*skencap;	/* sort order of sort key */
	dbchar	*dbencap;	/* sort order of sort key */
#endif
