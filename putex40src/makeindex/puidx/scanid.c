...
static int comp_len;
#ifdef DBCS
static dbchar key[ARGUMENT_MAX];
static dbchar dbbuf[ARGUMENT_MAX];
static int dbbuf_len = 0;
static dbchar dbnull = 0;
#else
...
static	void	make_dbstring ARGS((dbchar **ppstr,int n));
...
static	int	scan_field ARGS((int *n, int ck_level, int ck_encap,int ck_actual));
...
make_key(VOID_ARG)
{
    ...
    for (i = 0; i < FIELD_MAX; i++)
    {
		...
		ptr->data.sksf[i] = "";		/* initialize fields to pointers */;
		ptr->data.skaf[i] = "";		/* to constant empty strings */
		ptr->data.dbsf[i] = &dbnull;		/* initialize fields to pointers */;
		ptr->data.dbaf[i] = &dbnull;		/* to constant empty strings */

    }
    ptr->data.encap = "";
    ptr->data.skencap = "";
    ptr->data.dbencap = &dbnull;
    ptr->data.lpg[0] = NUL;
    ptr->data.count = 0;
    ptr->data.type = EMPTY;
...
static void
make_dbstring(dbchar **ppstr, int n)
{
    if ((*ppstr)[0] == 0)		/* then we have an empty dbstring */
    {
		(*ppstr) = (dbchar*)malloc(n*sizeof(dbchar));
		if ((*ppstr) == (dbchar*) NULL)
			FATAL("Not enough core...abort.\n", "");
		(*ppstr)[0] = 0;
    }
}
...
static void
dup_dbfield (dbchar *field)
{
	memcpy(field, dbbuf, sizeof(dbchar)*(dbbuf_len+1));
}

static void
dup_skfield (unsigned char *field)
{
	dbchar *dp = dbbuf;
...
scan_key(FIELD_PTR data)
{
    ...
    while (TRUE) {
		...
		if (key[n] == idx_encap)
		{
			...
			*/
			/* DBCS */
			isOK = scan_field(&n, FALSE, FALSE, FALSE);
			make_string(&(data->encap),dbbuf_len*2 + 1);
			dup_field(data->encap);
			make_string(&(data->skencap),dbbuf_len*2 + 1);
			dup_skfield(data->skencap);
			make_dbstring(&(data->dbencap),dbbuf_len*2 + 1);
			dup_dbfield(data->dbencap);
			if (isOK)
				break;
			else
				return (FALSE);
		...
		if (key[n] == idx_actual) {
			...
			if (i == last)
			{
				...
				if (!scan_field(&n, data->af[i], strlen(key),
					...
					return (FALSE);
				*/
				isOK = scan_field(&n, FALSE, TRUE, FALSE);
				make_string(&(data->af[i]),dbbuf_len*2 + 1);
				dup_field(data->af[i]);
				make_string(&(data->skaf[i]),dbbuf_len + 1);
				dup_skfield(data->skaf[i]);
				make_dbstring(&(data->dbaf[i]),dbbuf_len + 1);
				dup_dbfield(data->dbaf[i]);
			...
			else
			{
				...
				if (!scan_field(&n, data->af[i], strlen(key),
					...
					return (FALSE);
				*/
				isOK = scan_field(&n, TRUE, TRUE, FALSE);
				make_string(&(data->af[i]),dbbuf_len*2 + 1);
				dup_field(data->af[i]);
				make_string(&(data->skaf[i]),dbbuf_len + 1);
				dup_skfield(data->skaf[i]);
			...
			if (i == last)
			{
				...
				if (!scan_field(&n, data->sf[i], strlen(key),
					...
					return (FALSE);
				*/
				isOK = scan_field(&n, FALSE, TRUE, TRUE);
				make_string(&(data->sf[i]),dbbuf_len*2 + 1);
				dup_field(data->sf[i]);
				make_string(&(data->sksf[i]),dbbuf_len + 1);
				dup_skfield(data->sksf[i]);
				make_dbstring(&(data->dbsf[i]),dbbuf_len + 1);
				dup_dbfield(data->dbsf[i]);
				if (!isOK) return (FALSE);
			}
			else
			{
				/*
				make_string(&(data->sf[i]),strlen(key) + 1);
				if (!scan_field(&n, data->sf[i], strlen(key),TRUE, TRUE, TRUE))
					return (FALSE);
				*/
				isOK = scan_field(&n, TRUE, TRUE, TRUE);
				make_string(&(data->sf[i]),dbbuf_len*2 + 1);
				dup_field(data->sf[i]);
				make_string(&(data->sksf[i]),dbbuf_len + 1);
				dup_skfield(data->sksf[i]);
			...
			second_round = TRUE;
			/* DBCS: german_sort is not supported
...
static int
scan_field(int *n, int ck_level, int ck_encap, int ck_actual)
{
    int     i = 0;
    int     nbsh;		       /* backslash count */
	dbchar *field = dbbuf; /* DBCS */
	int len_field = ARGUMENT_MAX - 1; /* DBCS */
			...
			(key[*n] == NUL))
		{
			...
			if ((!ck_level) && (key[*n] == idx_level)) {
				IDX_ERROR2("Extra `%c' at position %d of first argument.\n",
					idx_level, *n + 1);
				goto FALSERET; /* DBCS: return (FALSE); */
...
/* DBCS: change GET_CHAR to GET_DBCHAR
*/
static int
scan_arg1(VOID_ARG)
{
    ...
    if (compress_blanks)
		while (((a = GET_DBCHAR(idx_fp)) == SPC) || (a == TAB));
		else
			a = GET_DBCHAR(idx_fp);
		...
		while ((i < ARGUMENT_MAX) && (a != EOF))
		{
			...
			{				/* take next character literally */
				key[i++] = (dbchar) a;	/* but preserve quote or escape */
				a = GET_DBCHAR(idx_fp);
				key[i++] = (dbchar) a;	/* save literal character */
			}
			else if (a == idx_aopen)
