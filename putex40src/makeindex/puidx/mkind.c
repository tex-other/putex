...
#if    OS_PCDOS
#if    IBM_PC_TURBO
unsigned	_stklen = 0xf000;	/* Turbo C ignores size set in .exe file by LINK or EXEMOD,	sigh... */
#endif					/* IBM_PC_TURBO */
#endif					/* OS_PCDOS */
...
#endif
{
	...
	path_list = miktex_get_config_value ("PUTeX", "ConfigPath", buf, sizeof (buf), 0);
	if (path_list == 0) {
		FATAL("Can't get puidx configuration value\n", "");
		exit(1);
	}
	if (! miktex_find_file ("puidx.sk", path_list, filenamebuf)) {
		FATAL("Can't open puidx's sort key file: puidx.sk\n", "");
		exit(1);
	}

	if ((fp = fopen(filenamebuf, "rb")) == NULL)
		FATAL("Can't open puidx's sort key file: puidx.sk\n", "");
	setup_dbcs_sk(fp);
#endif
		...
		if (**++argv == SW_PREFIX) {
			...
			for (ap = ++*argv; *ap != NUL; ap++)
				...
				case 'p':
					...
					/* enable german sort */
				case 'g':
					fprintf(stderr, "German sort is not supported by puidx\n"); 
					german_sort = FALSE;
					break;
...
NIL : WORK, base, INDEX_ILG);
		...
		* WORK-PVS */
		...
		if (log_given) {
			sprintf(log_fn, "%s%s", base, INDEX_LOG);
#if OS_PCDOS || OS_WIN32       		/* open in binary mode (CR-LF) */
...
#define DIR_DELIM '/'
...
#endif
		{
			...
			path = miktex_get_search_path ("Makeindex", STYLE_PATH, buf, sizeof(buf), 0);
			if (path == 0)
			{
			...
			{
				if (miktex_find_file (fn, path, sty_fn))
					sty_fp = OPEN_IN (sty_fn);
				else
					sty_fp = 0;
			}
