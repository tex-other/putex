%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
%%%  WEB Change File for PUTeX (Win32 CJK version)
%%%
%%%  Copyright (C) 1997-2004
%%%	 Chey_Woei Tsay <cwtsay@pu.edu.tw>
%%%
...
%%%  along with MiKTeX; if not, write to the Free Software
...
%% **    11  ******************************************************************
%% MikTex don't use this to set pool file.
%%@x
%%@!pool_name='[TeX]Format Dirs:tex.pool';
%%@y
%%@!pool_name='[TeX]Format Dirs:putex.pool';
%%@z
...
%%@d new_buf_size=3256 {TCW: must be greater than |buf_size_def|}
%%@z
...
%%@y
%%@<Declare the procedure called |print_dbchar|@>@;
%%@#
%%procedure print_char(@!s:ASCII_code); {prints a single character}
%%@z
...
%%  hi_byte:=so(str_pool[j]);
%%  if (hi_byte > 127 and j+1 < str_start[s+1]) then
%%    begin print_dbchar(hi_byte, so(str_pool[j+1])); j:=j+2;
%%    end
%%  else
%%    begin print_char(hi_byte); incr(j);
%%    end;
%%end;
%%@z
...
@y
and format identifier together will occupy at most |max_print_line|
character positions@^Input Encoding Dependencies@>.

@d putex_banner==' (PUTeX 4.0 CJK version)'
@z
...
%%@y
%%begin j:=str_start[str_ptr];
%%while j<pool_ptr do begin
%%  hi_byte:=so(str_pool[j]);
%%  if (hi_byte > 127 and j+1<pool_ptr) then
%%    begin print_dbchar(hi_byte, so(str_pool[j+1])); j:=j+2;
%%    end
%%  else
%%    begin print_char(hi_byte); incr(j);
...
%%end;
%%
%%@<Declare the procedure called |print_chinese_int|@>@;
%%@z
...
@y
      begin if font(p)<>font_in_short_display and font(p)<>cfont_in_short_display then
        begin if (font(p)<font_base)or(font(p)>cfont_max) then
          print_char("*")
@.*\relax@>
        else @<Print the font identifier for |font(p)|@>;
        print_char(" ");
...
@y
else  begin if (font(p)<font_base)or(font(p)>cfont_max) then print_char("*")
@z
...
%%@y
%%@d cspace_skip_base=skip_base+cfont_max_limit {table of cfont cspace}
%%@d cespace_skip_base=cspace_skip_base+cfont_max_limit {table of cfont cespace}
%%@d mu_skip_base=cespace_skip_base+256 {table of 256 ``muskip'' registers}
%%@z
...
@d int_pars=74 {total number of integer parameters}
@z
...
%%    while i < j do
%%      begin if i=loc then set_trick_count;
%%      if (buffer[i] > 127 and i+1 < j) then @^double byte code dependent@>
%%        begin print_dbchar(buffer[i],buffer[i+1]); i:=i+2;
%%        end
%%      else
%%        begin print(buffer[i]); incr(i);
%%        end
%%      end;
%%  end
%%@z
...
%%@y
%%begin get_token; {suppress macro expansion}
%%print("cur_tok = "); print_int(cur_tok);
%%print(" cur_chr = "); print_int(cur_chr);
%%@z
...
%%@y
%%@!dvi_gone:integer; {the number of bytes already output to |dvi_file|}
%%@!output_cdi:boolean; {flag to generate cdi output}
%%@z
...
@y
miktex_allocate_memory;
buf_size := buf_size - 256; { reserve the last 256 bytes for putex }
@z
...
%% **  1337  ******************************************************************
%%
%% terminal cjk input is not supported yet.
%@x
