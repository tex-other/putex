/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/type1.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
MEM_START
  ...
  /* The following section seems to determine which, if any,
     glyphs were used via the internal encoding, which hasn't
     been known until now. It was moved from do_pfb() to here 
     to avoid wasting a few kB in output size by adding the
     header, glyph description for /.notdef and the trailer
     of a pfb font which is not used at all. */
  if (partial_enabled) {
    int j;
    if (verbose > 2) {
...
/* If the only glyph in the font to be embedded is /.notdef, there's
 * no point of adding pfb_header and pfb_trailer wasting a few kByte 
 * per font */
	if ( pfbs[pfb_id].n_used_glyphs==1 && ! strncmp(pfbs[pfb_id].used_glyphs[0],".notdef",7)) {
		if (filtered) 
			RELEASE (filtered);
		length=0; /* which signifies that this font needs not be output. */
  }
  else {
    if (filtered) {	
...
static unsigned long do_pfb_body (FILE *file, int pfb_id)
{
  ...
  t1_crypt_init (EEKEY);
  for (i=0; i<length; i++) {
    buffer[i] = t1_encrypt((unsigned char)(partial_enabled ? filtered[i] : buffer[i]));
  }
  if (filtered)
    RELEASE (filtered);
  pdf_add_stream (pfbs[pfb_id].direct, (char *) buffer, length);
...
static void do_pfb (int pfb_id)
{
  ...
  if ( ! length1 ) {  /* no glyph is necessary in this font */ 
    if (verbose > 1)
      fprintf(stderr,"\n type1_fontfile %s doesn't have any glyph necessary\n",pfbs[pfb_id].pfb_name);
