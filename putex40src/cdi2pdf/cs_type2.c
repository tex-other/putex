/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/cs_type2.c,v 1.1 2003/12/27 18:03:16 csc Exp $
  ...
  *data += 1;
  ...
  case cs_endchar:
    if (stack_top == 1) {
      ...
      WARN("%s: Composite glyph not supported.", CS_TYPE2_DEBUG_STR);
			  ...
			  card8 **data, card8 *endptr)
{
  ...
  *data += 1;
  ...
  case cs_dotsection: /* deprecated */
    WARN("%s: Operator \"dotsection\" deprecated in Type 2 charstring.", CS_TYPE2_DEBUG_STR);
    status = CS_PARSE_ERROR;
    return;
    break;
  case cs_hflex:
