/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/cff.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
cff_font *cff_open(FILE *stream, long offset, int face)
{
  ...
  if (cff_dict_known(cff->topdict, "CharstringType") &&
    ...
    WARN("%s: Only Type 2 Charstrings supported.", CFF_DEBUG_STR);
    cff_close(cff);
    RELEASE(cff);
    return NULL;
  }
  ...
  if (cff_dict_known(cff->topdict, "SyntheticBase")) {
    WARN("%s: Synthetic font not supported.", CFF_DEBUG_STR);
    cff_close(cff);
    RELEASE(cff);
    return NULL;
  }
...
card16 cff_charsets_lookup (cff_font *cff, card16 code)
{
  ...
  case 1:
    for (i = 0; i < charset->num_entries; i++) {
      if (code >= (charset->data).range1[i].first &&
	  code <= (charset->data).range1[i].first + (charset->data).range1[i].n_left) {
        gid += code - (charset->data).range1[i].first + 1;
        break;
      }
      gid += (charset->data).range1[i].n_left + 1;
    }
    if (i == charset->num_entries) gid = 0;
    break;
  case 2:
    for (i = 0; i < charset->num_entries; i++) {
      if (code >= (charset->data).range2[i].first &&
	  code <= (charset->data).range2[i].first + (charset->data).range2[i].n_left) {
        gid += code - (charset->data).range2[i].first + 1;
        break;
      }
...
/* input: GID, output: SID/CID (card16) */
card16 cff_charsets_lookup_inverse (cff_font *cff, card16 gid)
{
  ...
  /* GID == 0 --> .notdef always have SID/CID 0 */
  if (gid == 0)
    return 0;
