/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/pkfont.c,v 1.1 2003/12/27 18:03:16 csc Exp $
	     ...
	     *res_name)
{
  ...
  if (i == num_pk_fonts) {
    ...
    if ((pk_file_name = kpse_find_glyph((char *)tex_name,
	...
	pdf_add_array (tmp1,
		       pdf_new_number(ROUND(tfm_get_width (tfm_font_id, j)*1000.0,1.)));
      }
      pdf_add_dict (pk_fonts[i].direct,
		    pdf_new_name ("Widths"),
		    tmp1);
...
static void embed_pk_font (int pk_id)
{
  ...
  if ((pk_file = MFOPEN (pk_fonts[pk_id].pk_file_name,
	...
	do_character ((unsigned char)pk_command_byte, pk_id, char_procs);
      } else  /* A command byte */
