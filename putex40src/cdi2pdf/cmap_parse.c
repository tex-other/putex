/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/cmap_parse.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
static int check_next_token  (ifreader *input, char *key);
...
static int
check_next_token (ifreader *input, char *key)
{
  int      cmp;
  pst_obj *token;
  char    *str;
...
#define MATCH_NAME(t,n) (PST_NAMETYPE((t))    && !strncmp(pst_data_ptr((t)),(n),strlen((n))))
#define MATCH_OP(t,n)   (PST_UNKNOWNTYPE((t)) && !strncmp(pst_data_ptr((t)),(n),strlen((n))))
  ...
  while (status >= 0) {
	...
	CMap_set_wmode(cmap, pst_getIV(tok2));
       ...
       */
      CIDSysInfo *csi = NEW(1, CIDSysInfo);
      csi->registry = NULL; csi->ordering = NULL; csi->supplement = -1;
      while ((tok2 = pst_get_token(&(input->cursor), input->endptr)) != NULL) {
	if (MATCH_OP(tok2, ">>") || MATCH_OP(tok2, "end")) {
	  pst_release_obj(tok2);
	  break;
	} else if (MATCH_NAME(tok2, "Registry") &&
		   (tok3 = pst_get_token(&(input->cursor), input->endptr)) != NULL &&
		   PST_STRINGTYPE(tok3) && check_next_token(input, "def") == 0)
	  csi->registry = pst_getSV(tok3);
	else if (MATCH_NAME(tok2, "Ordering") &&
		 (tok3 = pst_get_token(&(input->cursor), input->endptr)) != NULL &&
		 PST_STRINGTYPE(tok3) && check_next_token(input, "def") == 0)
	  csi->ordering = pst_getSV(tok3);
	else if (MATCH_NAME(tok2, "Supplement") &&
		 (tok3 = pst_get_token(&(input->cursor), input->endptr)) != NULL &&
		 PST_INTEGERTYPE(tok3) && check_next_token(input, "def") == 0)
	  csi->supplement = pst_getIV(tok3);
	if (tok3)
	  pst_release_obj(tok3);
	if (tok2)
	  pst_release_obj(tok2);
      }
      tok2 = tok3 = NULL;
      if (check_next_token(input, "def") < 0 ||
	  csi->registry == NULL || csi->ordering == NULL || csi->supplement < 0) {
	status = -1;
      } else {
