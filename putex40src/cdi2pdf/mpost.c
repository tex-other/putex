/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/mpost.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
static int n_mp_fonts = 0, max_mp_fonts = 0;
...
int mp_locate_font (char *tfm_name, double pt_size)
{
...
static int mp_parse_headers (char **start, char *end, struct xform_info *p)
{
  ...
  while (**start == '%') {
    if (strncmp(*start, "%%BoundingBox:", 14) == 0) { /* Found %%BoundingBox */
      *start += 14;
	  ...
	  (ury = parse_number(start, end))) {
        ...
        fprintf(stderr, "\nMissing expected four numbers in BoundingBox.\n");
        dump(*start, end);
      }
    }
    skip_line(start, end);
...
/* 
 * The following codes are for reading all %*Font: records in dvipdfm 0.13.2c.
 *
 * On the other hand, we skip those records in dvipdfmx.
...
static void flush_path (void)
{
  ...
  for (i=0; i<n_path_pts; i++) {
	...
	p = path[i].element;
	len = sprintf (work_buffer, "\n%g %g %g %g %g %g c",
		       ROUND((p->x)[0], .0001), ROUND((p->y)[0], .0001),
		       ROUND((p->x)[1], .0001), ROUND((p->y)[1], .0001),
		       ROUND((p->x)[2], .0001), ROUND((p->y)[2], .0001));
	pdf_doc_add_to_page (work_buffer, len);
	RELEASE (p);
      }
      break;
...
#define SETDASH		22
#define SETGRAY		23
/* SETLINECAP is already defined in wingdi.h of Microsoft SDK */
#define SETLINECAP_	24
  ...
  */
  ...
  case CONCAT:
	...
	len += sprintf (work_buffer+len, "%g",
			ROUND(pdf_number_value(tmp2), .0001));
      }
  ...
  case CURVETO:
    if (state <= 1) /* In path now */
      ...
      fprintf (stderr, "\nUnexpected path segment (curveto): state=%d\n", state);
    }
  ...
  case FSHOW: 
    {
	  ...
	  (tmp1 = POP_STACK()) &&
	  (tmp1->type == PDF_STRING || tmp1->type == PDF_STRINGX)) {
	fontid = mp_locate_font(pdf_string_value(tmp2), pdf_number_value(tmp3));
        if (mp_fonts[fontid].subfont_id >= 0) {
          unsigned int uch;
          unsigned char wch[2], *p = pdf_string_value(tmp1);
          int len = pdf_string_length(tmp1);
          while (len-- > 0) {
            uch = lookup_sfd_record(mp_fonts[fontid].subfont_id, *p++);
            wch[0] = uch >> 8;
            wch[1] = uch & 0xff;
            dev_set_string ((spt_t)(x_state/dvi_unit_size()),
                            (spt_t)(y_state/dvi_unit_size()),
                            wch, 2, 0, mp_fonts[fontid].font_id, 0);
          }
        } else
	  dev_set_string ((spt_t)(x_state/dvi_unit_size()),
                          (spt_t)(y_state/dvi_unit_size()),
  ...
  case LINETO: 
	...
	state = 1;
        ...
        fprintf (stderr, "\nUnexpected path segment (lineto): state=%d\n", state);
      }
      if ((tmp2 = POP_STACK()) && tmp2-> type == PDF_NUMBER &&
	  (tmp1 = POP_STACK()) && tmp1-> type == PDF_NUMBER) {
  ...
  case MOVETO:
    if (state <= 1) /* In path now */
      ...
      fprintf (stderr, "\nUnexpected path segment (moveto): state=%d\n", state);
    }
    if ((tmp2 = POP_STACK()) && tmp2-> type == PDF_NUMBER &&
	(tmp1 = POP_STACK()) && tmp1-> type == PDF_NUMBER) {
  ...
  case RLINETO: 
    if (state <= 1) /* In path now */
      ...
      fprintf (stderr, "\nUnexpected path segment (rlineto): state=%d\n", state);
    }
    if ((tmp2 = POP_STACK()) && tmp2->type == PDF_NUMBER &&
	(tmp1 = POP_STACK()) && tmp1->type == PDF_NUMBER) {
      x_state += pdf_number_value (tmp1);
  ...
  case SCALE: 
	...
	(tmp1 = POP_STACK()) && tmp1->type == PDF_NUMBER) {
      len = sprintf (work_buffer, "\n%g 0 0 %g 0 0 cm",
		     ROUND(pdf_number_value (tmp1), .01),
		     ROUND(pdf_number_value (tmp2), .01));
  ...
  case SETCMYKCOLOR:
	...
	(tmp1 = POP_STACK()) && tmp1->type == PDF_NUMBER) {
      len = sprintf (work_buffer, "\n%g %g %g %g k",
		     ROUND(pdf_number_value (tmp1), .01),
		     ROUND(pdf_number_value (tmp2), .01),
		     ROUND(pdf_number_value (tmp3), .01),
		     ROUND(pdf_number_value (tmp4), .01));
      pdf_doc_add_to_page (work_buffer, len);
      len = sprintf (work_buffer, "\n%g %g %g %g K",
		     ROUND(pdf_number_value (tmp1), .01),
		     ROUND(pdf_number_value (tmp2), .01),
  ...
  case SETDASH:
	...
	if ((tmp3 = pdf_get_array (tmp1, i)) &&
	  ...
	  len = sprintf (work_buffer, " %g", ROUND(pdf_number_value(tmp3), 1.));
	  pdf_doc_add_to_page (work_buffer, len);
	} else 
	  break;
      }
	...
	len = sprintf (work_buffer, " %g d", ROUND(pdf_number_value(tmp2), 1.));
	pdf_doc_add_to_page (work_buffer, len);
      }
    } else {
  ...
  case SETLINEWIDTH:
    if ((tmp1 = POP_STACK()) && tmp1->type == PDF_NUMBER) {
      len = sprintf (work_buffer, "\n%g w", ROUND(pdf_number_value (tmp1), .01));
      pdf_doc_add_to_page (work_buffer, len);
    } else {
  ...
  case TRANSLATE:
	...
	(tmp1 = POP_STACK()) && tmp1->type == PDF_NUMBER) {
      len = sprintf (work_buffer, "\n1 0 0 1 %g %g cm", 
		     ROUND(pdf_number_value (tmp1), .0001),
		     ROUND(pdf_number_value (tmp2), .0001));
      pdf_doc_add_to_page (work_buffer, len);
...
static int mp_parse_body (char **start, char *end,
			  double x_user, double y_user)
  ...
  while (*start < end && !error) {
    ...
    if (isdigit (**start) || **start == '-' || **start == '.' ) {
      ...
      PUSH (obj);
	       ...
	       (obj = parse_pdf_hex_string (start, end))) {
      PUSH (obj);
    } else if (**start == '/') {
		     ...
		     char *res_name, double x_user, double y_user)
{
  ...
  if ((size = file_size(image_file)) == 0) {
    fprintf (stderr, "Can't read any byte in the MPS file.\n");
    return NULL;
  }
  ...
  if (mp_parse_headers(&start, end, p)) {
    /* Looks like an MP file. Setup xobj "capture" */
    pdf_scale_image(p);
    if ((xobj = begin_form_xobj(p->u_llx, p->u_lly, p->c_llx, p->c_lly,
				p->c_urx, p->c_ury, res_name))) {
      /* prevent dev_set_string() to be disturbed by wmode */
      dev_start_mp_mode();
      top_stack = 0; x_state = 0.0; y_state = 0.0; state = 0;
      /* Flesh out the contents */
      if (!mp_parse_body(&start, end, 0.0, 0.0))
        fprintf(stderr, "\nErrors occured while interpreting MetaPost file.\n");
      /* prevent dev_set_string() to be disturbed by wmode */
      dev_end_mp_mode();
      /* Finish off the form */
      end_form_xobj(NULL);
