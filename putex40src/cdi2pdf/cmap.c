/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/cmap.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
/* Private funcs. */
static int  bytes_consumed   (CMap *cmap, unsigned char *instr, long inbytes);
...
static int  check_range      (CMap *cmap,
			      unsigned char *srclo, unsigned char *srchi, int srcdim, unsigned char *dst, int dstdim);

static unsigned char *get_mem        (CMap *cmap, int size);
static mapDef *mapDef_new     (void);
static void    mapDef_release (mapDef *t);
static int     locate_tbl     (mapDef **cur, unsigned char *code, int dim);
 ...
 */
static void
handle_undefined (CMap *cmap, unsigned char **inbuf, long *inbytesleft, unsigned char **outbuf, long *outbytesleft)
{
  long len = 0;
...
static void
CMap_decode_char (CMap *cmap, unsigned char **inbuf, long *inbytesleft, unsigned char **outbuf, long *outbytesleft)
{
  mapDef *t;
  unsigned char *p = *inbuf, c = 0;
  long    count = 0;

  if (__verbose > CMAP_DEBUG+10)
    MESG("%s: CMapType: %d, room for inbuf: %ld bytes, outbuf: %ld bytes\n",
	 CMAP_DEBUG_STR, cmap->type, *inbytesleft, *outbytesleft);

  /* Identity CMaps always mapps 16-bit code to 16-bit code */
  ...
  if (__verbose > CMAP_DEBUG+10)
    MESG("%s: mapTbl stopped at depth %d with flag %d.\n",
	 CMAP_DEBUG_STR, count, t[c].flag);
  ...
  if (LOOKUP_CONTINUE(t[c].flag)) /* need more bytes */
    ...
    if (cmap->useCMap) {
      if (__verbose > CMAP_DEBUG+10)
	MESG("%s: Lookup usecmap CMap %s\n",
	     CMAP_DEBUG_STR, CMap_get_name(cmap->useCMap));
...
long
CMap_decode (CMap *cmap, unsigned char **inbuf, long *inbytesleft, unsigned char **outbuf, long *outbytesleft)
{
  int count;
...
void
CMap_set_name (CMap *cmap, char *name)
{
  ASSERT(cmap);
  if (cmap->name)
    RELEASE(cmap->name);
...
CMap_set_CIDSysInfo (CMap *cmap, CIDSysInfo *csi)
{
  ...
  if (cmap->CSI) {
    if (__verbose > CMAP_DEBUG)
      WARN("%s: Trying re-define CIDSystemInfo.", CMAP_DEBUG_STR);
    if (cmap->CSI->registry) RELEASE(cmap->CSI->registry);
    if (cmap->CSI->ordering) RELEASE(cmap->CSI->ordering);
    RELEASE(cmap->CSI);
  }
...
CMap_set_usecmap (CMap *cmap, CMap *ucmap)
{
  ...
  if (__verbose > CMAP_DEBUG+10)
    MESG("\n%s: /%s usecmap\n", CMAP_DEBUG_STR, CMap_get_name(ucmap));
...
int
CMap_match_codespace (CMap *cmap, unsigned char *c, int dim)
{
  int i, pos;
 ...
 */
int
CMap_add_codespacerange (CMap *cmap, unsigned char *codelo, unsigned char *codehi, int dim)
{
  rangeDef *csr = NULL;
  int       i;
...
int
CMap_add_notdefchar (CMap *cmap, unsigned char *src, int srcdim, CID dst)
{
  return CMap_add_notdefrange(cmap, src, src, srcdim, dst);
}

int
CMap_add_notdefrange (CMap *cmap, unsigned char *srclo, unsigned char *srchi, int srcdim, CID dst)
{
  int     c;
  mapDef *cur;
  ...
  ASSERT(cmap);
  /* dst not used here */
  if (check_range(cmap, srclo, srchi, srcdim, (unsigned char *)&dst, 2) < 0)
    return -1;
...
int
CMap_add_bfchar (CMap *cmap, unsigned char *src, int srcdim, unsigned char *dst, int dstdim)
{
  return CMap_add_bfrange(cmap, src, src, srcdim, dst, dstdim);
}

int
CMap_add_bfrange (CMap *cmap, unsigned char *srclo, unsigned char *srchi, int srcdim, unsigned char *base, int dstdim)
{
  int     c;
  mapDef *cur;
...
int
CMap_add_cidchar (CMap *cmap, unsigned char *src, int srcdim, CID dst)
{
  return CMap_add_cidrange(cmap, src, src, srcdim, dst);
}

int
CMap_add_cidrange (CMap *cmap, unsigned char *srclo, unsigned char *srchi, int srcdim, CID base)
{
  int     c;
  mapDef *cur;
  ...
  ASSERT(cmap);
  /* base not used here */
  if (check_range(cmap, srclo, srchi, srcdim, (unsigned char *)&base, 2) < 0)
    return -1;
...
static int
locate_tbl (mapDef **cur, unsigned char *code, int dim)
{
  int i, c;
 ...
 */
static int
bytes_consumed (CMap *cmap, unsigned char *instr, long inbytes)
{
...
static int
check_range (CMap *cmap, unsigned char *srclo, unsigned char *srchi, int srcdim, unsigned char *dst, int dstdim)
{
...
static FILE *dpx_open_file (const char *cmap_name, int type);

#define DPX_RES_TYPE_CMAP 0
#define DPXFOPEN(n,t)     dpx_open_file((const char *)(n),(int)(t))
...
dpx_open_file (const char *filename, int type)
{
  ...
  if (!miktex_find_app_input_file("dvipdfm", filename, work_buffer)) {
    if (miktex_get_acrobat_font_dir(work_buffer)) {
      ...
      return MFOPEN(work_buffer, FOPEN_RBIN_MODE);
    }
  } else
    full_filename = work_buffer;
#else
  suffix = strrchr(filename, '.');
  if (suffix == NULL || strcmp(suffix+1, "cmap")) {
    filename_alt = NEW(strlen(filename)+6, char);
    strcpy(filename_alt, filename);
    strcat(filename_alt, ".cmap");
  } 
  full_filename = kpse_find_file(filename, kpse_program_text_format, 0);
  if (!full_filename && filename_alt)
    full_filename = kpse_find_file(filename_alt, kpse_program_text_format, 0);
  if (!full_filename) {
  ...
  if (__verbose > CMAP_DEBUG)
    MESG("\n%s: File \"%s\" found for \"%s\".\n", CMAP_DEBUG_STR, full_filename, filename);

  return MFOPEN(full_filename, FOPEN_RBIN_MODE);
}
#endif /* HAVE_DPXFILE */
...
CMap_cache_find (const char *cmap_name)
{
  ...
  for (id = 0; id < __cache->num; id++) {
    ...
    if (name && strcmp(cmap_name, name) == 0) {
      if (__verbose > CMAP_DEBUG)
	MESG("\n%s: CMap \"%s\" found in cache (id=%d).\n", CMAP_DEBUG_STR, cmap_name, id);
      return id;
    }
  }

  if (__verbose > CMAP_DEBUG)
    MESG("\n%s: CMap \"%s\" not found in cache (total=%d).\n", CMAP_DEBUG_STR,
	 cmap_name, __cache->num);
...
void
cmap_set_lookup (int flag)
{
  if (__verbose > 3)
    WARN("Obsolete function cmap_set_lookup() called.");
}
...
cmap_set_verbose (void)
{
  __verbose++;
  if (__verbose > 3)
    WARN("Obsolete function cmap_set_verbose() called.");
}

char *
cmap_cmapname (int id)
{
  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function cmap_cmapname() called.");
  return CMap_get_name(CMap_cache_get(id));
}
...
{
  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function cmap_wmode() called.");
  return CMap_get_wmode(CMap_cache_get(id));
}
...
cmap_sysinfo (int id)
{
  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function cmap_wmode() called.");
  return CMap_get_CIDSysInfo(CMap_cache_get(id));
}

unsigned short
cmap_lookup (unsigned char hi, unsigned char lo, int id)
{
  CMap   *cmap;
  CID     cid;
  long    inbytesleft = 2, outbytesleft = 32;
  unsigned char  inbuf[2], outbuf[32];
  unsigned char *inbufcur = inbuf, *outbufcur = outbuf;

  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function cmap_lookup() called.");
  inbuf[0] = hi; inbuf[1] = lo;
  cmap = CMap_cache_get(id);
  CMap_decode(cmap, &inbufcur, &inbytesleft, &outbufcur, &outbytesleft);

  if (inbytesleft != 0 || outbytesleft < 30)
    cid = 0;
  else
    cid = (outbuf[0] << 8)|outbuf[1];

  return (unsigned short) cid;
}

int
get_cmap (const char *cmap_name)
{
  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function get_cmap() called.");
  ASSERT(cmap_name);
  return CMap_cache_find(cmap_name);
}
...
{
  if (__verbose > CMAP_DEBUG+5)
    WARN("Obsolete function cmap_close_all() called.");
  CMap_cache_close();
}

#endif /* WITHOUT_CMAP_COMPAT */
