/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/pdfobj.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
static unsigned pdf_root_obj = 0, pdf_info_obj = 0, pdf_encrypt_obj = 0;
...
static void write_stringx (FILE *file, const pdf_string *string);
...
static void dump_trailer(void)
{
  ...
  if (pdf_encrypt_obj != 0) {
    unsigned char *id = pdf_enc_id_string();
...
void pdf_out_flush (void)
{
  if (pdf_output_file) {
#ifdef DEBUG
    fprintf (stderr, "pdf_obj_out_flush:  dumping xref\n");
#endif
    dump_xref();
#ifdef DEBUG
    fprintf (stderr, "pdf_obj_out_flush:  dumping trailer\n");
#endif
    dump_trailer();
    if (verbose) {
...
void pdf_set_encrypt (pdf_obj *object)
{
  if (pdf_encrypt_obj != 0) {
    ERROR ("pdf_set_encrypt:  encrypt object already set");
  }
  if (object -> label == 0) {  /* Make sure this object has a label */
    pdf_label_obj (object);
  }
...
pdf_obj *pdf_new_stringx (const void *string, unsigned length)
{
  pdf_obj *result = pdf_new_string (string, length);
  result->type = PDF_STRINGX;
  return result;
}
...
static void write_string (FILE *file, const pdf_string *string)
{
  ...
  int count, i;
  pdf_encrypt_data (s, string->length);
  pdf_out_char (file, '(');
...
static void write_stringx (FILE *file, const pdf_string *string)
{
  ...
  pdf_out_char (file, '<');
  for (i=0; i<string->length; i++) {
    sprintf(format_buffer, "%02x", s[i]);
    pdf_out (file, format_buffer, 2);
  }
  pdf_out_char (file, '>');
}
...
void pdf_set_stringx (pdf_obj *object, unsigned char *string, unsigned length)
{
  object -> type = PDF_STRINGX;
  pdf_set_string (object, string, length);
}
...
void pdf_write_obj (FILE *file, const pdf_obj *object)
{
  ...
  case PDF_STRING:
    ...
    break;
  case PDF_STRINGX:
    write_stringx (file, object -> data);
    break;
  case PDF_NAME:
...
#ifdef DEBUG
      fprintf (stderr, "pos: %lu gen: %u\n", xref_table[i].position, xref_table[i].generation);
#endif
