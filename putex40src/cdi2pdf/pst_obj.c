/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/pst_obj.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
typedef struct { unsigned char *value; }  pst_name;
typedef struct
{
  long    length;
  unsigned char *value;
...
/* NAME */
static pst_name *pst_name_new      (unsigned char *name) ;
...
static unsigned char   *pst_name_data_ptr (pst_name *obj);
...
static unsigned char     *pst_string_data_ptr (pst_string *obj)       ;
...
{
  ASSERT(obj);
  return obj->value ? strdup("true") : strdup("false");
}
...
{
  if (*inbuf + 4 <= inbufend && strncmp(*inbuf, "true", 4) == 0
      && PST_TOKEN_END(*inbuf + 4, inbufend)) {
    *inbuf += 4;
    return pst_new_obj(PST_TYPE_BOOLEAN, pst_boolean_new(1));
  } else if (*inbuf + 5 <= inbufend && strncmp(*inbuf, "false", 5) == 0
...
{
  if (*inbuf + 4 <= inbufend && strncmp(*inbuf, "null", 4) == 0 &&
      PST_TOKEN_END(*inbuf+4, inbufend)) {
    *inbuf += 4;
...
{
  unsigned char fmtbuf[PST_MAX_DIGITS+1];
  ASSERT(obj);
  sprintf(fmtbuf, "%ld", obj->value);
...
{
  unsigned char fmtbuf[PST_MAX_DIGITS+5];
  ASSERT(obj);
  sprintf(fmtbuf, "%.5g", obj->value);
  return strdup(fmtbuf);
}
...
static pst_name *
pst_name_new (unsigned char *name)
{
  pst_name *obj;
  obj = NEW(1, pst_name);
  obj->value = strdup(name);
  return obj;
}
...
int
pst_name_is_valid (unsigned char *name)
{
...
unsigned char *
pst_name_encode (unsigned char *name)
{
  unsigned char wbuf[PST_NAME_LEN_MAX*3+1];
  int    i, len = strlen(name);
  unsigned char c, *p = wbuf;
  ...
  for (i = 0; i < len; i++) {
    c = name[i];
    if (c == 0)
      WARN("Invalid char: \\0 not allowed for name object. (ignored)");
    else if (c < '!' || c > '~' ||
...
pst_parse_name (unsigned char **inbuf, unsigned char *inbufend) /* / is required */
{
  ...
  *inbuf = cur;
  return pst_new_obj(PST_TYPE_NAME, pst_name_new(wbuf));
}
