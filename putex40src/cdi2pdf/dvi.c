/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/dvi.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
static UNSIGNED_PAIR num_pages = 0, stackdepth;
...
{
  int thisfont, map_id, subfont_id, font_id;
  char *tex_name;

#ifdef DEBUG
  fprintf(stderr, "dvi_locate_font: fontname: (%s) ptsize: %ld, dvi_id: %d\n", tfm_name, ptsize, num_loaded_fonts);
#endif
  if (verbose)
    fprintf(stderr, "<%s@%.2fpt", tfm_name, ptsize*dvi2pts);
  ...
  loaded_fonts[thisfont].tfm_id = tfm_open(tfm_name, 1); /* must exist */
  loaded_fonts[thisfont].subfont_id = subfont_id;
  loaded_fonts[thisfont].size = ptsize;
  /* This will be reset later if it was really generated by the dvi file. */
  loaded_fonts[thisfont].source = VF;

  /* The order of searching fonts is as follows:
   *
   * 1. If map_id is negative, that is, there is no map entry matching
   *    with tfm_name, then search a virtual font matching with tfm_name
   *    at first. If no virtual font is found, search a PK font matching
   *    with tfm_name.
   *
   * 2. If map_id is non-negative, search a physical scalable font.
   *
   * 3. Notice that every subfont gets non-negative map_id. In this case,
   *    enc_name corresponding to map_id will be used instead of map_id.
   ...
   */
  if (map_id < 0) {
    if ((font_id = vf_locate_font(tfm_name, ptsize)) >= 0) {
      loaded_fonts[thisfont].type = VIRTUAL;
      loaded_fonts[thisfont].font_id = font_id;
      if (verbose) fprintf(stderr, "(VF)>");
      return thisfont;
    }
  } else if (subfont_id >= 0 && fontmap_enc_name(map_id) == NULL) {
    if ((font_id = vf_locate_font(fontmap_font_name(map_id), ptsize)) >= 0) {
      loaded_fonts[thisfont].type = VIRTUAL;
      loaded_fonts[thisfont].font_id = font_id;
      if (verbose) fprintf(stderr, "(OVF)>");
      return thisfont;
    } else {
      fprintf(stderr, "\n** Fatal: Could not locate a Omega Virtual Font file for '%s'.", tfm_name);
      ERROR ("\n");
    }
  }
  /* Failed to load a virtual font so that try to load a physical font. */
  tex_name = (map_id < 0 ? tfm_name : fontmap_tex_name(map_id));
  if ((font_id = dev_locate_font(tex_name, ptsize, map_id)) < 0) {
    fprintf(stderr, "** Fatal error: Could not locate a virtual or a physical font file for '%s'.", tfm_name);
    ERROR ("\n");
  }
...
static void do_string (unsigned char *s, int len)
{
  ...
  switch (p->type) {
  case PHYSICAL:
    if (p->subfont_id >= 0) { /* For subfonts, call dvi_set as VF fonts */
      dvi_push();
      for (i = 0; i < len; i++) dvi_set (s[i]);
      dvi_pop();
      break;
    }
...
void dvi_set (SIGNED_QUAD ch)
{
  ...
  case PHYSICAL:
    if (ch > 255) {
      wch[0] = (unsigned char)(ch >> 8);
      wch[1] = ch & 0xff;
      dev_set_string (dvi_state.h, -dvi_state.v, wch, 2, width,
		      p->font_id, 2);
    } else if (p->subfont_id >= 0) {
      unsigned int uch = lookup_sfd_record(p->subfont_id, (unsigned char)ch);
      wch[0] = uch >> 8;
      wch[1] = uch & 0xff;
      dev_set_string (dvi_state.h, -dvi_state.v, wch, 2, width,
		      p->font_id, 2);
...
void dvi_rule (SIGNED_QUAD width, SIGNED_QUAD height)
{
  ...
  if (!dvi_state.d) {
    ...
    dev_rule (dvi_state.h, -dvi_state.v-width,
	      height, width);
  }
}

void dvi_dir (UNSIGNED_BYTE dir)
{
  dvi_state.d = dir;
  dev_set_wmode((int) dvi_state.d); /* 0: horizontal, 1: vertical */
}
...
#ifdef DEBUG
  fprintf(stderr, "Seeking to page %d @ %ld\n", n, page_loc[n]);
#endif
...
int dvi_init (char *dvi_filename, char *pdf_filename, double mag, double x_offset, double y_offset)
{
...
void dvi_paper_size (double *width, double *height, char *landscape)
{
  unsigned char opcode;

  seek_absolute(dvi_file, page_loc[0]);

  while ((opcode = fgetc(dvi_file)) != EOP) {
    if (opcode <= SET_CHAR_127 || (opcode >= FNT_NUM_0 && opcode <= FNT_NUM_63))
      continue;
    switch (opcode) {
      case BOP: case NOP: case PUSH: case POP: case W0: case X0: case Y0: case Z0: case DIR:
        break;
      case SET1: case PUT1: case RIGHT1: case DOWN1: case W1: case X1: case Y1: case Z1: case FNT1:
        fgetc(dvi_file);
	break;
/* mod -- cdi2pdf */
      case CFNT: case SET2: case PUT2: case RIGHT2: case DOWN2: case W2: case X2: case Y2: case Z2: case FNT2:
/* end -- cdi2pdf */
        seek_relative(dvi_file, 2);
	break;
      case SET3: case PUT3: case RIGHT3: case DOWN3: case W3: case X3: case Y3: case Z3: case FNT3:
        seek_relative(dvi_file, 3);
	break;
      case SET4: case PUT4: case RIGHT4: case DOWN4: case W4: case X4: case Y4: case Z4: case FNT4:
        seek_relative(dvi_file, 4);
	break;
      case SET_RULE: case PUT_RULE:
        seek_relative(dvi_file, 8);
	break;
/* add -- cdi2pdf */
...
/* end -- cdi2pdf */
	...
	break;
      case XXX1:
      { char *start, *end, buffer[256];
	int size = fgetc(dvi_file) & 0xFF;
	if (fread(buffer, 1, size, dvi_file) != size)
	  ERROR("DVI file ended prematurely");
        start = buffer; end = buffer + size;
        skip_white(&start, end);
        if (strncmp(start, "landscape", 9) == 0)
          *landscape = 1;
        else if (strncmp(start, "papersize", 9) == 0) {
          char *key, *val, *val_start, *val_end, *num;
          double unit;
          parse_key_val(&start, end, &key, &val);
          if (key && val) {
            val_start = val; val_end = val + strlen(val);
            skip_white(&val_start, val_end);
            if ((num = parse_number(&val_start, val_end))) {
              *width = atof(num); RELEASE(num);
      ...
      case XXX2:
       	seek_relative(dvi_file, get_unsigned_pair(dvi_file));
	break;
      case XXX3:
       	seek_relative(dvi_file, get_unsigned_triple(dvi_file));
	break;
      case XXX4:
       	seek_relative(dvi_file, get_unsigned_quad(dvi_file));
	break;
