/*  $Header: /cvsroot/miktex/miktex/dvipdfmx/pdfdoc.c,v 1.1 2003/12/27 18:03:16 csc Exp $
...
static void start_page_tree (void)
{
  ...
  /*
   * FlateEncode-ing short text is not good. But I will do this.
   */
  glob_page_bop = pdf_new_stream(STREAM_COMPRESS);
  glob_page_eop = pdf_new_stream(STREAM_COMPRESS);
...
#define BANNER "%s (%s), Copyright \251 2002 by Jin-Hwan Cho and Shunsaku Hirata, Copyright \251 1998, 1999 by Mark A. Wicks"
...
static void finish_docinfo(void)
{
  ...
  banner = NEW (strlen(BANNER)+strlen(PACKAGE)+strlen(VERSION), char);
  sprintf(banner, BANNER, PACKAGE, VERSION);
...
static void finish_page_tree(void)
{
     ...
     other pages inherit it */
  {
    ...
    pdf_add_array (tmp1, pdf_new_number (0));
    pdf_add_array (tmp1, pdf_new_number (ROUND(dev_base_page_width(), .001)));
    pdf_add_array (tmp1, pdf_new_number (ROUND(dev_base_page_height(), .001)));
...
#define CLUSTER 4
  ...
  if (is_root)
    ...
    pdf_add_array(limit_array, pdf_new_string(dests[0].name, dests[0].length)); 
    pdf_add_array(limit_array, pdf_new_string(dests[ndests-1].name, dests[ndests-1].length));
  }
  if (ndests > 0 && ndests < 2*CLUSTER) {
    name_array = pdf_new_array();
    pdf_add_dict(result, pdf_new_name("Names"), name_array);
    for (i = 0; i < ndests; i++) {
      pdf_add_array(name_array, pdf_new_string(dests[i].name, dests[i].length));
...
MEM_START
		...
		pdf_link_obj(contents_name), tmp1);
  {
    ...
    if (height) {
      tmp1 = pdf_new_array();
      pdf_add_array(tmp1, pdf_new_number(0));
      pdf_add_array(tmp1, pdf_new_number(height));
      pdf_add_array(tmp1, pdf_new_number(ROUND(dev_page_width(), .001)));
      pdf_add_array(tmp1, pdf_new_number(ROUND(dev_base_page_height(), .001)));
      pdf_add_dict(pages[page_count].page_dict, pdf_link_obj(mediabox_name), tmp1);
    }
  }
				   ...
				   double d, double e, double f)
{
  ...
  pdf_add_array (result, pdf_new_number (ROUND(e, .001)));
  pdf_add_array (result, pdf_new_number (ROUND(f, .001)));
  return result;
}
			  ...
			  double bburx, double bbury, char *res_name)
{
  ...
  /* Translate coordinate system so reference point of object 
     is at 0 */
  bbox = pdf_new_array ();
  pdf_add_array (bbox, pdf_new_number (ROUND(bbllx, .001)));
  pdf_add_array (bbox, pdf_new_number (ROUND(bblly, .001)));
  pdf_add_array (bbox, pdf_new_number (ROUND(bburx, .001)));
  pdf_add_array (bbox, pdf_new_number (ROUND(bbury, .001)));
...
void end_form_xobj (pdf_obj *resources)
{
  if (xobjects_pending > 0) {
    xobjects_pending -= 1;
    dev_close_all_xforms(res_stack[xobjects_pending].xform_depth);
    if (this_page_xobjects) {
      pdf_add_dict(current_page_resources, pdf_new_name("XObject"), pdf_ref_obj(this_page_xobjects));
      pdf_release_obj(this_page_xobjects);
    }
    if (this_page_fonts) {
      pdf_add_dict(current_page_resources, pdf_new_name("Font"), pdf_ref_obj(this_page_fonts));
      pdf_release_obj(this_page_fonts);
    }
    if (resources)
      pdf_merge_dict(current_page_resources, resources);
    ...
    /* Must reselect the font again in case there was a font change in the object */
    dev_reselect_font();
    /* Must reselect color too */
    dev_do_color();
  } else
    fprintf(stderr, "[doc] end_form_xobj: Tried to close a nonexistent xobject\n");
}
...
void pdf_doc_flush_annot (void)
{
  ...
  if (breaking_state.dirty) {
    ...
    pdf_add_array (rectangle, pdf_new_number(ROUND(breaking_state.llx-grow, .001)));
    pdf_add_array (rectangle, pdf_new_number(ROUND(breaking_state.lly-grow, .001)));
    pdf_add_array (rectangle, pdf_new_number(ROUND(breaking_state.urx+grow, .001)));
    pdf_add_array (rectangle, pdf_new_number(ROUND(breaking_state.ury+grow, .001)));
