...
unsigned long fc[MAXFONTS]; /* checksum    */
unsigned long fs[MAXFONTS]; /* size        */
unsigned long fd[MAXFONTS]; /* design size */
...
/* rewritten 1995/June/30 */
void do_put1(FILE *input)
{
  unsigned int c;
  c = ureadone(input);
  printf("\nput1: %u ", c);
...
void extension (char *fname, char *str)
{
  char *s, *t;
...
void forceexten (char *fname, char *str)
{
    ...
    ((t = strrchr(fname, '\\')) != NULL && s < t))
  {
    ...
    strcpy(s+1, str);    /* give it default extension */
}
...
//printf("DVIDUMP 1.1 Copyright (C) 2000 Y&Y, Inc. http://www.YandY.com\n");
  printf("DVIDUMP 1.2 Copyright (C) 2014 Clerk Ma. \n");
  printf("\n");
