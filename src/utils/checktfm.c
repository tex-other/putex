...
/* positions of emdash in that encoding */
int emdashes[] =
{
  151,
  151,
...
/* get emdash char code, given encoding name */
int emdashcode(char *name)
{
  int k;
  for (k = 0; k < 16; k++)
  {
    if (strcmp(encodenames[k], "") == 0)
      break;
...
/* should be removed to a standalone library */
...
/* 2^20 = 1048576 */
 /* map to tfm scaled integer */
long mapdouble(double x)
{
  if (x == 0.0)
    return 0L;
  else if (x < 0.0)
    ...
    return (long) ((x * 1048576.0 + 499.999) / 1000.0);
}
...
/* read TFM file to get info including em_quad */
int checktfm(FILE *input)
{
  int k, kem;
  long charinfo;
  int nchars, nwidths;
...
int main(int argc, char *argv[])
{
  ...
  while (*argv[firstarg] == '-')
  {
    if (strcmp(argv[firstarg], "-v") == 0)
      ...
      doubleflag = 1;
    else if (strcmp(argv[firstarg], "-m") == 0)
      modifyflag = 1;
    firstarg++;
  }
  if (modifyflag)
    printf("WARNING: Will modify TFM files\n");
